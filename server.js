...
const styleProfiles = {
  marketplace: {
    name: "Маркетплейс PRO",
    accent: "#0B63C9",
    accent2: "#173B6D",
    text: "#111820",
    panel: "#FFFFFF",
    scene: "Сильная продающая карточка маркетплейса: крупный товар, дорогой рекламный свет, мягкий lifestyle-фон с глубиной, чистые белые и цветные панели, круглые и капсульные акценты, визуальная иерархия как у профессиональных карточек Ozon и Wildberries."
  },
  minimal: {
    name: "Минимализм",
    accent: "#F03E4A",
...
const GIDEON_COGNITION_MODES = {
  decision: "Проанализируй решение: варианты, плюсы, минусы, риски, неизвестные, критерии выбора и что проверить до решения. Не выбирай за пользователя.",
  scenario: "Построй 3-5 сценариев: базовый, оптимистичный, негативный и при необходимости альтернативные. Для каждого дай предпосылки, последствия, риски и ранние сигналы.",
  compare: "Сравни объекты по фактам и критериям пользователя. Покажи совпадения, отличия, компромиссы и неизвестные данные без произвольного победителя.",
  conflicts: "Найди противоречия, несовместимые утверждения, разные цифры/даты/условия. Для каждого конфликта укажи обе версии и что нужно проверить.",
  duplicates: "Найди полные и смысловые дубликаты, почти одинаковые записи и повторяющиеся факты. Сгруппируй их и объясни различия.",
  fact_check: "Проверь утверждения только против переданного контекста. Для каждого укажи: подтверждено, опровергнуто, недостаточно данных или конфликт источников, с кратким основанием.",
  questions: "Сформируй список важных открытых вопросов, которые мешают завершить задачу или принять обоснованное решение. Приоритизируй по влиянию.",
  next_action: "Предложи 1-5 следующих практических действий, исходя из текущего состояния, зависимостей и рисков. Не заявляй о выполнении действий.",
  focus: "Сожми контекст до одной главной цели на сегодня, критических задач, блокеров и того, что можно отложить.",
  goal_review: "Оцени прогресс к цели по доступным фактам: сделано, осталось, блокеры, риски, следующий контрольный пункт.",
  daily_brief: "Сделай ежедневный бриф: что изменилось, что требует внимания, завершенные задачи, approvals, риски и приоритеты на сегодня.",
  evening_brief: "Сделай вечерний итог: что завершено сегодня, что изменилось, что осталось открытым, риски и что перенести на завтра.",
  weekly_review: "Сделай недельный обзор: прогресс проектов, ключевые решения, незавершенные вопросы, риски, новые знания и приоритеты следующей недели.",
  research_notebook: "Оформи исследовательскую записку: вопрос, факты, источники/опоры из контекста, противоречия, выводы, неизвестные и дальнейшее исследование.",
  source_map: "Построй карту утверждений к основаниям: для каждого важного вывода укажи, чем он подтверждается в переданном контексте. Не выдумывай источники.",
  evidence: "Раздели утверждения на подтвержденные факты, сильные выводы, слабые выводы, предположения и неизвестное. Объясни основание кратко.",
  timeline: "Построй хронологию событий по датам. Если точная дата неизвестна, явно укажи неопределенность.",
  people: "Выдели людей и организации: роль, связи, упоминания, связанные проекты/документы/события и неизвестные сведения.",
  translation: "Переведи материал на целевой язык, сохранив структуру, числа, имена и смысл. Не добавляй новых фактов.",
  document: "Создай полноценный структурированный документ: заголовок, executive summary, разделы, таблицы при необходимости, выводы и приложения. Текст должен быть готов для экспорта в DOCX.",
  report: "Создай профессиональный отчет: executive summary, контекст, факты, анализ, риски, выводы и следующие действия.",
  presentation: "Создай структуру презентации с 8-12 слайдами: заголовок, тезис, 3-5 пунктов и рекомендуемая визуализация на каждом слайде.",
  spreadsheet: "Спроектируй таблицу: листы, колонки, типы данных, формулы/расчеты и пример строк. Если контекст содержит данные — нормализуй их в табличную структуру.",
  dashboard: "Спроектируй аналитический dashboard: KPI, формулы, источники данных, фильтры, графики и правила интерпретации.",
  form: "Спроектируй форму: поля, типы, обязательность, валидация, условная логика и итоговый объект данных.",
  miniapp: "Подготовь спецификацию мини-приложения: цель, пользователи, экраны, данные, действия, состояния, ошибки и минимальный HTML/JS прототип при уместности.",
  critic: "Критически проверь материал: слабые места, пропущенные допущения, внутренние противоречия, риски и конкретные улучшения.",
  second_opinion: "Дай независимый альтернативный анализ, специально не повторяя исходную логику. Отдели факты от интерпретаций.",
  consensus: "Смоделируй три независимые профессиональные позиции, затем выдели совпадения, разногласия и наиболее устойчивые выводы без искусственного консенсуса.",
  meeting: "Оформи протокол встречи: участники если известны, темы, решения, задачи, ответственные, сроки, открытые вопросы и краткое резюме.",
  scanner: "Определи тип материала, извлеки текст и факты, затем предложи 3-5 подходящих действий с ним.",
...
app.post("/api/gideon/cognition", async (req, res) => {
  ...
  if (limitMap(neuroHubGigaIpLimits, "gideon-cognition:" + ip, 30)) {
    return res.status(429).json({ error: "Слишком много запросов Function Studio.", code: "free_rate_limit" });
  }
  const mode = String(req.body?.mode || "").trim();
  if (!GIDEON_COGNITION_MODES[mode]) return res.status(400).json({ error: "Неизвестный режим Function Studio.", code: "invalid_mode" });
  const input = String(req.body?.input || "").trim().slice(0, 22000);
  const context = String(req.body?.context || "").trim().slice(0, 16000);
  const targetLanguage = String(req.body?.targetLanguage || "Русский").slice(0, 80);
  if (!input && !context) return res.status(400).json({ error: "Нет материала для обработки.", code: "input_missing" });
  try {
    const result = await neuroHubUltraCompletion([
      { role: "system", content: [
        "Ты Gideon Function Studio 7.0.",
        GIDEON_COGNITION_MODES[mode],
        mode === "translation" ? "Целевой язык: " + targetLanguage : "",
        "Всегда различай факты, выводы и неизвестное. Не изображай внешние действия выполненными."
      ].filter(Boolean).join("\n") },
      { role: "user", content: [input ? "Материал:\n" + input : "", context ? "Контекст:\n" + context : ""].filter(Boolean).join("\n\n") }
    ], { maxTokens: 2200, temperature: ["critic","conflicts","evidence","source_map"].includes(mode) ? 0.18 : 0.35 });
    neuroHubGigaDaily.count += 1;
    return res.json({ mode, answer: result.content, protocol: GIDEON_PROTOCOL_VERSION, usage: gideonUsageSnapshot() });
  } catch (error) {
    return res.status(502).json({ error: "Function Studio временно недоступен.", code: error?.code || "cognition_failed" });
  }
});
...
app.post("/api/generate-cards", async (req, res) => {
  ...
  try {
    ...
    const referenceScenes = [null, null, null, null];
    let aiImageCalls = 0;

    if (gigaChatConfigured()) {
      try {
        productReferenceFileId = await withTimeout(
          uploadImageToGigaChat(sourceBuffer, mimeType, 0),
          24000,
        ...
        for (const index of [0, 1]) {
          try {
            ...
            console.warn("Yuvion Studio reference scene fallback:", {
              index,
              message: error?.message,
              status: error?.status,
              code: error?.code,
        ...
        if (referenceScenes[0]) {
          referenceScenes[2] = await makeGigaBackgroundVariant(
            referenceScenes[0],
            2,
            renderPalette
          );
        }
        if (referenceScenes[1]) {
          referenceScenes[3] = await makeGigaBackgroundVariant(
            referenceScenes[1],
            3,
            renderPalette
          );
        ...
        console.warn("Yuvion Studio reference generation unavailable:", {
          message: error?.message,
          status: error?.status,
          code: error?.code,
          details: error?.details
    ...
    stats.aiSceneRenders += aiImageCalls;

    let coverOptimization = {
      tested: 1,
    ...
    // If the cover was created from the product reference, keep the generated composition intact.
    if (!referenceScenes[0]) {
      try {
        const selected = renderSelections[0];
        const candidateVariants = [0,1,2,3];
        const candidates = [];

        for (const candidateVariant of candidateVariants) {
          const profile = candidateVariant === variant
            ? studioProfile
            : buildStudioProfile(normalized, styleKey, candidateVariant, primaryAspect);
          const scene = candidateVariant === variant
            ? scenes[0]
            : await renderFreeScene(
                selected.primary.buffer,
                0,
                styleKey,
                renderPalette,
                candidateVariant,
          ...
          const cover = await composeCard(
            cached,
            overlayForCard(0, normalized, styleKey, renderPalette, intensity, substyle, visual, profile)
          );
          const metrics = await localCardVisualMetrics(cover);
          const score = coverVisualScore(metrics);
    ...
    const cachedScenes = await Promise.all(
      scenes.map((scene) => normalizeSceneForCache(scene))
    );
    ...
    return res.json({
      ...
      gigaChatImageGeneration: aiImageCalls > 0,
      referenceGuidedGeneration: aiImageCalls > 0,
      referenceSceneCount: aiImageCalls,
    ...
    return imageErrorResponse(req, res, error, requestLimitMap, "batch-yuvion-studio");
  } finally {
