...
app.get("/m", async (_req,res) => {
  ...
  res.send(mobileShell("Главная", `
    ...
    <div class="card muted">Эта версия адаптирована и для компьютера, и для телефона.</div>
  `,{scripts:`<script>
...
app.get("/", (_req, res) => res.redirect(302, "/m"));
app.get("/pamyat-juhuro", (req, res) => {
  // Unified responsive interface: the former desktop SPA lost required sections
  // and could throw on button clicks. Keep legacy only for explicit diagnostics.
