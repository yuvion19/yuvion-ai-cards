document.addEventListener("DOMContentLoaded",()=>{
  ...
  render("#red-village-media",D.redVillageMedia||[],x=>`<figure class="media-card"><img loading="lazy" src="${x.image}" alt="${safe(x.title)}"><figcaption><h3>${safe(x.title)}</h3><p>${safe(x.credit)} · ${safe(x.license)}</p><a class="text-link" target="_blank" rel="noopener" href="${x.source}">Wikimedia Commons ↗</a></figcaption></figure>`);
