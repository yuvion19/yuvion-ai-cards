...
document.getElementById('fill').onclick=async()=>{
  try{
    ...
    if(payload?.type!=='yuvion-product-autofill'||!payload.product)throw new Error('В буфере нет пакета Yuvion AI.');
    ...
    status('Готово. Проверьте форму, подтвердите категорию и добавьте изображения.');
