...
function openTool(name){
  ...
  if(name==="timeline"){
    ...
    body.innerHTML=`<div class="row wrap"><button id="route20">20 минут</button><button id="route60">1 час</button><button id="heritageRoute">Полдня</button><button id="clearRoute">Очистить</button><button id="playRoute" class="primary">▶ Экскурсия</button></div><div id="routeItems"></div>`;renderRouteItems();$("route20").onclick=()=>makeTimedRoute(20);$("route60").onclick=()=>makeTimedRoute(60);$("heritageRoute").onclick=()=>makeTimedRoute(180);$("clearRoute").onclick=clearRoute;$("playRoute").onclick=startTour;
...
function meterDist(a,b){const R=6371000,p1=a[1]*Math.PI/180,p2=b[1]*Math.PI/180,dp=(b[1]-a[1])*Math.PI/180,dl=(b[0]-a[0])*Math.PI/180,h=Math.sin(dp/2)**2+Math.cos(p1)*Math.cos(p2)*Math.sin(dl/2)**2;return 2*R*Math.asin(Math.sqrt(h))}
function buildRoadGraph(){if(state.roadGraph)return state.roadGraph;const adj=new Map(),coords=new Map(),key=p=>p[0].toFixed(6)+","+p[1].toFixed(6),add=(a,b)=>{const ka=key(a),kb=key(b),w=meterDist(a,b);coords.set(ka,a);coords.set(kb,b);if(!adj.has(ka))adj.set(ka,[]);if(!adj.has(kb))adj.set(kb,[]);adj.get(ka).push([kb,w]);adj.get(kb).push([ka,w])};for(const f of state.data?.roads?.features||[]){const a=f.geometry.coordinates;for(let i=0;i<a.length-1;i++)add(a[i],a[i+1])}state.roadGraph={adj,coords};return state.roadGraph}
function nearestRoadNode(p){const g=buildRoadGraph();let best=null,bd=Infinity;for(const [k,c] of g.coords){const d=meterDist(p,c);if(d<bd){bd=d;best=k}}return best}
function heapPush(h,item){h.push(item);let i=h.length-1;while(i){const p=(i-1)>>1;if(h[p][0]<=item[0])break;h[i]=h[p];i=p}h[i]=item}
...
function shortestRoadPath(a,b){const g=buildRoadGraph(),s=nearestRoadNode(a),goal=nearestRoadNode(b);if(!s||!goal)return{coords:[a,b],distance:meterDist(a,b)};const dist=new Map([[s,0]]),prev=new Map(),heap=[];heapPush(heap,[0,s]);while(heap.length){const [d,u]=heapPop(heap);if(d!==dist.get(u))continue;if(u===goal)break;for(const [v,w] of g.adj.get(u)||[]){const nd=d+w;if(nd<(dist.get(v)??Infinity)){dist.set(v,nd);prev.set(v,u);heapPush(heap,[nd,v])}}}if(!dist.has(goal))return{coords:[a,b],distance:meterDist(a,b)};const keys=[];let cur=goal;while(cur){keys.push(cur);if(cur===s)break;cur=prev.get(cur)}keys.reverse();return{coords:[a,...keys.map(k=>g.coords.get(k)),b],distance:dist.get(goal)+meterDist(a,g.coords.get(s))+meterDist(g.coords.get(goal),b)}}
function orderHeritage(limit=20){const candidates=(state.data?.buildings?.features||[]).filter(heritage),left=[...candidates],ordered=[];let pos=[map.getCenter().lng,map.getCenter().lat];while(left.length&&ordered.length<limit){let bi=0,bd=Infinity;left.forEach((f,i)=>{const d=meterDist(pos,centerOf(f));if(d<bd){bd=d;bi=i}});const f=left.splice(bi,1)[0];ordered.push(f.properties.qqId);pos=centerOf(f)}return ordered}
function makeTimedRoute(minutes=180){const ordered=orderHeritage(30);if(!ordered.length)return toast("Нет объектов с историческими тегами");const selected=[];let distance=0,prev=[map.getCenter().lng,map.getCenter().lat];for(const id of ordered){const next=centerOf(buildingById(id)),seg=shortestRoadPath(prev,next);const nextDistance=distance+seg.distance,estimate=nextDistance/75+(selected.length+1)*4;if(selected.length&&estimate>minutes)break;selected.push(id);distance=nextDistance;prev=next}state.route=selected.length?selected:[ordered[0]];state.routeMeta={distance,minutes:Math.round(distance/75+state.route.length*4),targetMinutes:minutes};store.routes=[{title:"Маршрут "+minutes+" мин",points:[...state.route],distanceMeters:Math.round(distance),estimatedMinutes:state.routeMeta.minutes,updatedAt:new Date().toISOString()}];localStorage.setItem(STORE_KEY,JSON.stringify(store));drawRoute();renderRouteItems();toast(`Маршрут: ${state.route.length} точек · ≈ ${(distance/1000).toFixed(1)} км · ${state.routeMeta.minutes} мин`)}
function makeHeritageRoute(){makeTimedRoute(180)}
function drawRoute(){if(map.getLayer("route-line"))map.removeLayer("route-line");if(map.getSource("route"))map.removeSource("route");if(state.route.length<2)return;let coords=[],distance=0;for(let i=0;i<state.route.length-1;i++){const a=centerOf(buildingById(state.route[i])),b=centerOf(buildingById(state.route[i+1])),seg=shortestRoadPath(a,b);distance+=seg.distance;if(coords.length)seg.coords.shift();coords.push(...seg.coords)}state.routeMeta={...(state.routeMeta||{}),distance,minutes:Math.round(distance/75+state.route.length*4)};map.addSource("route",{type:"geojson",data:{type:"Feature",geometry:{type:"LineString",coordinates:coords},properties:{distanceMeters:Math.round(distance)}}});map.addLayer({id:"route-line",type:"line",source:"route",paint:{"line-color":"#e9a66f","line-width":4,"line-dasharray":[2,1]}})}
function clearRoute(){state.route=[];state.routeMeta=null;store.routes=[];localStorage.setItem(STORE_KEY,JSON.stringify(store));if(map.getLayer("route-line"))map.removeLayer("route-line");if(map.getSource("route"))map.removeSource("route");renderRouteItems();stopTour()}
function renderRouteItems(){const el=$("routeItems");if(!el)return;el.innerHTML=state.route.length?`<div class="sourceItem"><b>≈ ${((state.routeMeta?.distance||0)/1000).toFixed(1)} км · ${state.routeMeta?.minutes||"—"} мин</b><small>Маршрут построен по локальной дорожной сети OSM.</small></div>`+state.route.map((id,i)=>`<button class="routeItem" data-route-id="${id}"><b>${i+1}. ${esc(label(buildingById(id)))}</b><small>${esc(id)}</small></button>`).join(""):'<div class="sourceItem">Маршрут пока пуст.</div>';el.querySelectorAll("[data-route-id]").forEach(b=>b.onclick=()=>selectHouse(b.dataset.routeId))}
