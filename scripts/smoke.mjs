...
if(!html.includes("new Worker('/local-vision-worker.js?v=10.5.0'"))fail('local vision worker hook missing');else ok('local vision worker hook present');
...
if(!html.includes("register('/sw.js?v=19',{updateViaCache:'none'})"))fail('service worker forced update missing');else ok('service worker forced update present');
...
if(!html.includes("local-vision-worker.js?v=10.5.0"))fail('vision worker cache bust missing');else ok('vision worker cache bust present');
...
if(!html.includes("register('/sw.js?v=19'"))fail('service worker v19 registration missing');else ok('service worker v19 registration present');
if(!sw.includes("yuvion-ai-shell-v19"))fail('service worker v19 cache missing');else ok('service worker v19 cache present');
