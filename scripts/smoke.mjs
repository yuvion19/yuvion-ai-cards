...
const features=[
  ...
  ['copyProviderPriority: ["openai","groq","cloudflare","openrouter","local"]','copy provider priority metadata'],
...
if(!html.includes("register('/sw.js?v=26',{updateViaCache:'none'})"))fail('service worker forced update missing');else ok('service worker forced update present');
...
if(!html.includes("register('/sw.js?v=26'"))fail('service worker v26 registration missing');else ok('service worker v26 registration present');
if(!sw.includes("yuvion-ai-shell-v26"))fail('service worker v26 cache missing');else ok('service worker v26 cache present');
